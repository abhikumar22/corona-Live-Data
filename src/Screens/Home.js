  ...
  sortArray(type) {
    ...
    if (type === 1) {
      ...
      if (type === 2) {
        ...
        users.sort(function (a, b) {
          return sortType === 0
            ? parseInt(a.cases.replace(/[^\w ]/, "")) -
                parseInt(b.cases.replace(/[^\w ]/, ""))
            : parseInt(b.cases.replace(/[^\w ]/, "")) -
                parseInt(a.cases.replace(/[^\w ]/, ""));
        });
        ...
        users.sort(function (a, b) {
          return sortType === 0
            ? a.region.replace(/[^\w ]/, "") - b.region.replace(/[^\w ]/, "")
            : b.region.replace(/[^\w ]/, "") - a.region.replace(/[^\w ]/, "");
        });
      } else if (type === 4) {
        users.sort(function (a, b) {
          return sortType === 0
            ? a.deaths.replace(/[^\w ]/, "") - b.deaths.replace(/[^\w ]/, "")
            : b.deaths.replace(/[^\w ]/, "") - a.deaths.replace(/[^\w ]/, "");
        });
      } else if (type === 5) {
        users.sort(function (a, b) {
          return sortType === 0
            ? a.total_recovered.replace(/[^\w ]/, "") -
                b.total_recovered.replace(/[^\w ]/, "")
            : b.total_recovered.replace(/[^\w ]/, "") -
                a.total_recovered.replace(/[^\w ]/, "");
        });
      } else if (type === 6) {
        users.sort(function (a, b) {
          return sortType === 0
            ? a.new_deaths.replace(/[^\w ]/, "") -
                b.new_deaths.replace(/[^\w ]/, "")
            : b.new_deaths.replace(/[^\w ]/, "") -
                a.new_deaths.replace(/[^\w ]/, "");
        });
      } else if (type === 7) {
        users.sort(function (a, b) {
          return sortType === 0
            ? a.new_cases.replace(/[^\w ]/, "") -
                b.new_cases.replace(/[^\w ]/, "")
            : b.new_cases.replace(/[^\w ]/, "") -
                a.new_cases.replace(/[^\w ]/, "");
        });
      } else if (type === 8) {
        users.sort(function (a, b) {
          return sortType === 0
            ? a.serious_critical.replace(/[^\w ]/, "") -
                b.serious_critical.replace(/[^\w ]/, "")
            : b.serious_critical.replace(/[^\w ]/, "") -
                a.serious_critical.replace(/[^\w ]/, "");
        });
      } else if (type === 9) {
        users.sort(function (a, b) {
          return sortType === 0
            ? a.active_cases.replace(/[^\w ]/, "") -
                b.active_cases.replace(/[^\w ]/, "")
            : b.active_cases.replace(/[^\w ]/, "") -
                a.active_cases.replace(/[^\w ]/, "");
        });
      } else {
        users.sort(function (a, b) {
          return sortType === 0
            ? a.total_cases_per_1m_population.replace(/[^\w ]/, "") -
                b.total_cases_per_1m_population.replace(/[^\w ]/, "")
            : b.total_cases_per_1m_population.replace(/[^\w ]/, "") -
                a.total_cases_per_1m_population.replace(/[^\w ]/, "");
        });
      }
              ...
              <thead>
                <tr className="th">
                  ...
                  >
                    ...
                    {/* <span>
                      {" "}
                      <img alt="new" src={require("../assests/arrow.png")} />
                    </span> */}
                  </th>
                  <th
                    style={{ cursor: "pointer" }}
